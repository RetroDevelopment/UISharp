...
public class InstanceCreator(Application application, TypeMapper typeMapper, IEAMLBinder? binder = null)
{
    ...
    public UIComponent CreateUIComponent(Component component)
    {
        ...
        var arguments = PrepareConstructorArguments(constructor.GetParameters(), GetAllComponentsRecursively(childComponentInstances), component);
    ...
    private List<UIComponent> GetAllComponentsRecursively(List<UIComponent> childComponentInstances)
    {
        var result = new List<UIComponent>();
        foreach (var component in childComponentInstances)
        {
            result.Add(component);
            var componentChilren = (List<UIComponent>)typeof(UIComponent).GetField("_children", BindingFlags.NonPublic | BindingFlags.Instance)!.GetValue(component)!;
            result.AddRange(GetAllComponentsRecursively(componentChilren));
        }
