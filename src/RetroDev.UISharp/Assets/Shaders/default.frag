...
void main()
{
    ...
    // If mask is 0 gl_FragDept = 1.0, that is, the fragment will be clipped and it must not overwrite the buffer to avoid transparency artifacts
    // If the fragment is fully transparent (FragColor.a == 0) same thing, because otherwise the depth buffer value would prevent from rendering solid colors below this fragment
